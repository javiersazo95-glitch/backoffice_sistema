...
export interface CapturerProfile { id:number; usuarioId:number; nombre:string; rut:string; email:string; telefono:string; alias:string; fotoPerfil?:string|null; regionId:number; region:string; comunaId:number; comuna:string; estado:CapturerStatus; activo:boolean; gananciaMes:number; motivoRechazo?:string|null; codigoReferido?:string|null; antecedentesNombre?:string|null; carnetFrenteNombre?:string|null; carnetReversoNombre?:string|null; createdAt:string; }
export interface CapturerMovement { id:number; tipo:string; estado:string; montoBase:number; montoComision:number; descripcion:string; fecha:string; negocioNombre?:string|null; }
...
export interface CapturerBankData { rut?:string|null; banco?:string|null; bankCode?:number|null; tipoCuenta?:string|null; numeroCuenta?:string|null; titular?:string|null; email?:string|null; }
export interface CapturerDashboard { perfil:CapturerProfile; pendiente:number; disponible:number; pagado:number; total:number; casas:number; publicidad:number; casasAprobadas:number; serviciosAprobados:number; conversiones:number; movimientos:CapturerMovement[]; retiros:CapturerWithdrawal[]; datosBancarios?:CapturerBankData|null; }
...
export interface CapturerChat { id:number; nombreNegocio:string; tipoNegocio:string; ultimoMensaje?:string|null; actualizadoEn:string; }
